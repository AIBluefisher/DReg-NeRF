...
class TransformerCrossEncoderLayer(nn.Module):
    def __init__(
        ...
        self.self_attn = nn.MultiheadAttention(pos_emb_dim, num_heads, dropout=dropout)
        self.cross_attn = nn.MultiheadAttention(pos_emb_dim, num_heads, dropout=dropout)

        # Implementation of Feedforward model
        self.linear1 = nn.Linear(pos_emb_dim, dim_feedforward)
        self.dropout = nn.Dropout(dropout)
        self.linear2 = nn.Linear(dim_feedforward, pos_emb_dim)

        self.norm1 = nn.LayerNorm(pos_emb_dim)
        self.norm2 = nn.LayerNorm(pos_emb_dim)
        self.norm3 = nn.LayerNorm(pos_emb_dim)
    ...
    def forward_post(
        ...
        src2, satt_weights_s = self.self_attn(
            q, k, value=src_w_pos if self.sa_val_has_pos_emb else src,
            # attn_mask=src_mask,
        ...
        src2, xatt_weights_s = self.cross_attn(
            query=self.with_pos_embed(src, src_pos),
            key=tgt_w_pos,
            value=tgt_w_pos if self.ca_val_has_pos_emb else tgt,
            # attn_mask=tgt_mask,
            key_padding_mask=tgt_key_padding_mask
        )
        tgt2, xatt_weights_t = self.cross_attn(
    ...
    def forward_pre(
        ...
        src3, xatt_weights_s = self.cross_attn(
            query=self.with_pos_embed(src2, src_pos),
            key=tgt_w_pos,
            value=tgt_w_pos if self.ca_val_has_pos_emb else tgt2,
            # attn_mask=tgt_mask,
            key_padding_mask=tgt_key_padding_mask
        )
        tgt3, xatt_weights_t = self.cross_attn(
            query=self.with_pos_embed(tgt2, tgt_pos),
            key=src_w_pos,
        ...
        # Stores the attention weights for analysis, if required.
        self.attn_weights = (satt_weights_s, satt_weights_t)
        self.xatt_weights = (xatt_weights_s, xatt_weights_t)
