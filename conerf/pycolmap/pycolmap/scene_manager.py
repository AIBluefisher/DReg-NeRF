...
class SceneManager(object):
    ...
    def __init__(self, colmap_results_folder, image_path=None, data_type="COLMAP"):
    ...
    def _load_images_bin(self, input_file):
        self.images = OrderedDict()
        offset = 1 if self.data_type == 'DAGSFM' else 0
        ...
        with open(input_file, 'rb') as f:
            ...
            image_struct = struct.Struct('<I I 4d 3d I') if self.data_type == 'DAGSFM' else \
                           struct.Struct('<I 4d 3d I')
            for _ in range(num_images):
                data = image_struct.unpack(f.read(image_struct.size))
                image_id = data[0]
                # cluster_id = data[1]
                q = Quaternion(np.array(data[1+offset:5+offset]))
                t = np.array(data[5+offset:8+offset])
                camera_id = data[8+offset]
            ...
            for line in iter(lambda: f.readline().strip(), ''):
                ...
                if is_camera_description_line:
                    ...
                    image = Image(data[-1], int(data[-2]),
                                  Quaternion(np.array(map(float, data[1+offset:5+offset]))),
                                  np.array(map(float, data[5+offset:8+offset])))
                else:
